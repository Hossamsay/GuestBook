...
class PostDetail extends Component {
    ...
    showCommentBox(){
        if($this.state.author!==""){
            return <div>
                    <textarea id="comment" placeHolder="Say Something" className="form-control"></textarea><br/>
                    <button className="btn btn-success" onClick={$this.saveComments}>Reply</button>
                   </div>
        }        
    }
  render() {
      	...
      	<div>
            <div className="row">
                <div className="col-md-12">
                    ...
                    {this.showTags()}
                    <br/>
                    <p>Message : {$this.state.post.description}</p>
                    {this.showCommentBox()}
                </div>
            </div>
            <div className="row">
